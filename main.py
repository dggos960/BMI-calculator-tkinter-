...
class BMI_Calculator(App):
    def build(self):
        ...
        lbl_heigh = Image(source="weight_lbl.png",size_hint=(.210,.050),pos=(125,1080))
        self.layout.add_widget(lbl_heigh)
        self.input_heigh = TextInput(text="1",pos=(380,1030),size_hint=(.230,.030),halign="center")
        ...
        lbl_weigh = Image(source='height_lbl.png',size_hint=(.210,.050),pos=(460,1080))
        self.layout.add_widget(lbl_weigh)           
        self.input_weigh = TextInput(text="1",pos=(30,1030),size_hint=(.230,.030),halign="center")
    ...
    def on_click_inputbox(self,instance,value) :
        ...
        if (self.input_heigh.text)=='1'and (self.is_float_convertable(str(self.input_heigh.text))==False)  :
            self.spinner_h.set_disabled(True)
        if (self.input_weigh.text)=='1' and (self.is_float_convertable(str(self.input_weigh.text))==False) :
            self.spinner_w.set_disabled(True)
        if (self.input_weigh.text) != '1' and (self.is_float_convertable(str(self.input_weigh.text))==True) :        
            self.spinner_w.set_disabled(False)
        if (self.input_heigh.text)!='1' and (self.is_float_convertable(str(self.input_heigh.text))==True):
    ...
    def calculate_action(self, value):
        ...
        if not self.is_float(self.input_heigh.text) or not self.is_float(self.input_weigh.text):
            ...
            lbl_error = Button(text='Please select gender (male or female)', color=(1, 0, 0, 1), size_hint=(.57, .03), background_color=(1, 1, 1, 1), pos=(300, 950))
            self.layout.add_widget(lbl_error) 
        elif self.spinner_h.text=='select\nparameter' or self.spinner_w.text=='select\nparameter':
            lbl_error = Button(text='Please select both parameters', color=(1, 0, 0, 1), size_hint=(.57, .03), background_color=(1, 1, 1, 1), pos=(300, 950))
            self.layout.add_widget(lbl_error)             
        
        else:
            lbl_error = Button(text='Done', color=(0, 1, 0, 1), size_hint=(.62, .03), background_color=(1, 1, 1, 1), pos=(270, 950))
            self.layout.add_widget(lbl_error)             
            ...
            if gender == 'male':
                ...
                if float(bmi) < 18.5:
                    ...
                    btn4 = Image(source='overweight_female.png', pos=(0, 400),size_hint=(.45,.05))
